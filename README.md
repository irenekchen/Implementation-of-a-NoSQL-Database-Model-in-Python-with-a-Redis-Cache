 ...
 * The node types are: Fan, Player, Team, Comment.
   * The relationship types are:
     ...
     * Comments:
       ...
       * The create_comment(), create_sub_comment(), get_player_comments(), and get_team_comments() methods handle these queries.
       
Redis cache for faster lookup:
* Check the Redis cache before executing a query. The function will return the cached value if it exists, and not query the database. (Known as a cache hit)
* If retrieve_by_template() does not get a cache hit (called a cache miss), it runs the query, inserts the result in the cache and returns the query result.
* The retrieve_by_template() function will print a message on a cache hit, cache miss and cache insert.
