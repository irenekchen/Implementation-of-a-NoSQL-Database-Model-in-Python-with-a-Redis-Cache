...
def check_query_cache(resource, template, fields):
   ...
   # mid_results = []
    #cur, results = r.scan(0, "")

    #print("Printing...")
    
    #for k in r.hscan(resource)
    key = compute_key(resource, template, fields)
    #print("computed key", key, fields)
    #for key in r.scan_iter("people*"):
        #r.get(key)
        #print(key)
    #print(r.exists(key))
    if r.exists(key):
...
def add_to_query_cache(resource, template, fields, query_result):
    # Batting:yearID=2004,teamID=BOS,playerID=ortizda01,f=AB,H,HR,playerID 
    # Batting:yearID=2004,teamID=BOS,playerID=ortizda01,f= playerID,AB,H,HR
    # Batting:teamID=BOS,playerID=ortizda01,yearID=2004,f=AB,H,HR,playerID
    #"Batting:teamID=BOS,playerID=ortizda01,yearID=2004,f= , ,,,,,,,A,B,D,H,H,I,R,a,e,l,p,r,y"
    """
    ...
    """
    field = fields[1:-1].split(",")
    key = compute_key(resource, template, field)
    add_to_cache(key, query_result[0])

    #print("added new key", key, field)
    return key
